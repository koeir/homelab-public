Arch Linux installation _guide_ for a minimal home server with LUKS encrypted
disk (not including `boot` and `efi`) + TPM2 auto-unlock.

> **[DISCLAIMER]**
> This guide assumes that your machine is compatible with TPM2.
> For checking compatibility with TPM2, read the documentation on _ArchWiki_.
...
1. Connect to the network with `iwctl` or `mmcli`.
   - If you're having trouble with `iwctl` regarding SSIDs and special
     characters, check out `installation/troubleshooting/iwctl_ssid_trouble.md`
...
> [!NOTE]
> `{port}` is a placeholder for a port number!! 2. Restart `sshd.service` until the listening port updates.
...
3. Generate key-pairs on local machine (if haven't already) transfer the public
   key.
   > [!TIP]
...
> The key pair generated on the local machine \_(that is to say **not** the

                                              machine you are currently
                                              installing Arch on)_ can be reused later when Arch is installed.

## Disk Partitioning and Formatting

1. Make partitions for boot, efi, and main filesystem using any partitioning
   tool.
   - Partition 1 (efi):
     1G
   - Partition 2 (boot):
     1G
   - Partition 3 (main filesystem):
     remaining (929.5G in my case)
...
> [!NOTE]
> `lvm` (found in `cryptsetup open` line), `volgroup0` (found in `vgcreate` line), and volumes starting with `lv_` are \*conventional

                                                                                                                       names*. That is to say that they can be replaced with whatever you want.
...
1. Mount the respective partitions and swap
   ...
   - `swap` (or in this case, `lv_swap`) should be mounted with `swapon
{swap_partition}`

2. Run `pacstrap -K base /mnt linux linux-firmware` to install base packages.
   - An error stating that `/etc/vconsole.conf` is not found might appear after
...
1. Add necessary hooks
   - `sd-encrypt lvm2` ALWAYS after `block`, and before `filesystems`, **in
     that order**.
   - `sd-encrypt` is **necessary** for `TPM2` to work, among other things.
2. Run `mkinitcpio -P` or `mkinitcpio -p linux`.
   - If it is looking for file `/etc/vconsole.conf`, it is likely empty or does
     not exist.
   - If it is either of the conditions above, just run `echo "KEYMAP=us >
/etc/vconsole.conf`
     - or whatever keymap you would like.

### Customizations

1. Set time and locale.
   - Timezone can be set with `ln -sf /usr/share/zoneinfo/{Area}/{Location}
/etc/localtime`
   - locale can be set in `/etc/locale.gen` by uncommenting a locale (e.g.
     `en_us.UTF-8 UTF-8`) and running `locale-gen`
...
2. Edit `/etc/hostname` accordingly.
...
2. Mount the first partition (efi)
   - `systemctl daemon-reload` might need to be called first; this command must
     be ran in the installer environment
3. Install grub/efi
   - `grub-install --target=x86_64-efi --bootloader-id=grub_uefi --recheck`
4. Copy locale to grub
...
2. Enroll a `TPM2` key for the `LUKS` encrypted partition.
   ...
   - Research for the optimal parameters.
     If I'm correct, `--tpm2-device=auto` + default paremeters is sufficient
     for functionality.
